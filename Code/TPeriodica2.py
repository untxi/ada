class Elemento:
    ...
    def __repr__(self):
        ...
        tira = "Elemento -------> [%s] \n" % self.nombre + \
               ...
               "Número Atómico -> [%d] \n" % self.nAtomico + \
               "Masa -----------> [%3.4f] \n" % self.masa + \
               "Grupo, Periodo -> [%d, %d]" % (self.grupo, self.periodo)
        return tira

    def perteneceSerie(self, sQ):
        """ Verifica si la instancia pertenece a la serie
            química indicada por sQ.
            Entradas:
...
def caso(nn, param = None):
    ...
    if nn == 1:    
        ...
        p = Elemento("Selenio", "Se", 34, 78.96, 16, 4, 5)
        ...
        Elemento("Plata","",47,107.8682,11,5,6)
        #------------------------------------------------------------#
